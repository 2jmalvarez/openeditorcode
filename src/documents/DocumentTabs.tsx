  ...
  return <box style={{ flexShrink: 0, flexDirection: "row", flexWrap: "wrap", backgroundColor: "#111820" }}>
    <For each={props.tabs()}>{(tab, index) => (
      <box onMouseDown={() => props.onActivate(index())} style={{ paddingX: 1, flexShrink: 0, flexDirection: "row", backgroundColor: index() === props.activeTab() ? "#263a46" : "#111820" }}>
        <text fg={index() === props.activeTab() ? tab.kind === "diff" ? "#70d6a7" : tab.kind === "logs" ? "#e68b8b" : "#f2c66d" : "#8ca0ae"}>{tab.kind === "diff" ? "Δ " : tab.kind === "manual" ? "? " : tab.kind === "image" ? "▣ " : tab.kind === "logs" ? "! " : props.isTabDirty(index()) ? "* " : ""}{basename(tab.path)}{tab.kind === "diff" && tab.diff.revision ? ` @ ${tab.diff.revision.slice(0, 8)}` : ""}</text>
